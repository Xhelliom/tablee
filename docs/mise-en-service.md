...
| `GEMINI_API_KEY` | Facultatif. Un repas décrit avec l'IA reçoit l'image de son plat, dessinée par Gemini 3.1 Flash Lite Image à l'enregistrement ; absent, sa carte garde le bol dessiné. Même conseil de limite de dépense, côté Google AI Studio (dette n° 21). Le modèle se change par `TABLEE_IMAGE_MODEL`. |
