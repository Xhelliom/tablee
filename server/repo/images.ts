/**
 * Les images de plats dessinées par le modèle (015), une par ensemble
 * d'ingrédients et par foyer. Pourquoi par foyer et en base : en-tête de la
 * migration.
 */
import type { HouseholdDb } from '../db.ts';
import type { DishImage } from '../llm/image.ts';

/** Où l'écran lit l'image : derrière la session, comme le reste de l'API. */
export const dishImageUrl = (id: string): string => `/api/images/${id}`;

/** Pose sur le repas l'image du foyer qui porte cette étiquette, et rend son identifiant ; `null` s'il n'y en a pas. */
export async function reuseDishImage(
  db: HouseholdDb, householdId: string, mealId: string, tag: string,
): Promise<string | null> {
  const { rows } = await db.query<{ image_id: string | null }>(
    `update meal set image_id = (select id from dish_image where household_id = $1 and tag = $2)
     where household_id = $1 and id = $3
     returning image_id`,
    [householdId, tag, mealId],
  );
  return rows[0]?.image_id ?? null;
}

/**
 * Garde une image, la pose sur le repas, et rend son identifiant ; `null` si
 * le repas a disparu pendant le dessin. Deux repas aux mêmes ingrédients
 * dessinés en même temps : la première image gagne, la seconde est jetée.
 */
export async function saveDishImage(
  db: HouseholdDb, householdId: string, mealId: string, tag: string, image: DishImage,
): Promise<string | null> {
  const { rows } = await db.query<{ image_id: string | null }>(
    `with image as (
       insert into dish_image (household_id, tag, mime_type, bytes) values ($1, $2, $3, $4)
       on conflict (household_id, tag) do update set tag = excluded.tag
       returning id
     )
     update meal set image_id = (select id from image)
     where household_id = $1 and id = $5
     returning image_id`,
    [householdId, tag, image.mimeType, image.bytes, mealId],
  );
  return rows[0]?.image_id ?? null;
}

export async function loadDishImage(db: HouseholdDb, id: string): Promise<DishImage | null> {
  const { rows } = await db.query<{ mime_type: string; bytes: Buffer }>(
    'select mime_type, bytes from dish_image where id = $1',
    [id],
  );
  const row = rows[0];
  return row === undefined ? null : { mimeType: row.mime_type, bytes: row.bytes };
}
