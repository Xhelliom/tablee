/**
 * V3 — l'assistant (hors roadmap d'origine, 14/09/2026).
 *
 * La route rassemble ce que l'assistant a le droit de savoir, en retire les
 * prénoms, et transmet. Ce qu'il sait et ce qu'il refuse : en-tête de
 * `server/llm/conseil.ts`. Rien n'est écrit en base.
 */
...
export function assistantRoutes(app: FastifyInstance, ctx: AppContext): void {
  app.post('/api/assistant', { config: { rateLimit: LLM_RATE_LIMIT } }, async (request) => {
    ...
    const names = namesToHide(everyone, identity.name);
    const facts = anonymize(describeHousehold({
